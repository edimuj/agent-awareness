...
import { codexHooksInstall, type HooksScope } from './codex-hooks.ts';
import { ensureServer } from '../daemon/client.ts';
...
export async function codexLive(options: CodexLiveOptions = {}): Promise<void> {
...
async function waitForAppServer(listenUrl: string, child: ChildProcess): Promise<void> {
...
async function waitForHookHandshake(runDir: string, timeoutMs: number): Promise<HookHandshake | null> {
  const path = join(runDir, 'session-start-handshake.json');
  const deadline = Date.now() + timeoutMs;
  while (Date.now() < deadline) {
    if (existsSync(path)) {
      try {
        return JSON.parse(readFileSync(path, 'utf8')) as HookHandshake;
      } catch {
        return null;
      }
    }
...
function spawnFallbackSidecar(runDir: string, env: NodeJS.ProcessEnv): number {
...
function cleanupRun(runDir: string, appServer: ChildProcess): void {
  ...
  try { appServer.kill('SIGTERM'); } catch { /* already gone */ }
}
...
function appendLauncherLog(runDir: string, message: string): void {
