...
import { LEGACY_MCP_ENTRY_KEY, MCP_ENTRY_KEY } from './codex-mcp.ts';
...
export interface ParsedCodexMcpEntry {
  key: string;
  command: string | null;
  args: string[];
}

export interface CodexMcpSmokeResult {
  ok: boolean;
  toolCount: number;
...
export async function runCodex(args: string[]): Promise<CommandResult> {
...
export async function getInstalledCodexMcpEntry(): Promise<CodexMcpEntry | null> {
  const primary = await runCodex(['mcp', 'get', MCP_ENTRY_KEY]);
  if (primary.code === 0) return { key: MCP_ENTRY_KEY, stdout: primary.stdout };
  if (primary.code !== 1) {
  ...
  if (legacy.code === 0) return { key: LEGACY_MCP_ENTRY_KEY, stdout: legacy.stdout };
  if (legacy.code !== 1) {
...
export function parseCodexMcpEntry(entry: CodexMcpEntry): ParsedCodexMcpEntry {
...
export function resolveConfiguredScriptPath(parsed: ParsedCodexMcpEntry, cwd: string): string | null {
...
export async function smokeCodexMcpTools(
  command: string,
  args: string[],
  cwd: string,
): Promise<CodexMcpSmokeResult> {
