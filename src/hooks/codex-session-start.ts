...
function existingAlivePid(pidPath: string): number | null {
  ...
  if (!Number.isFinite(pid) || !isAlive(pid)) return null;
  return pid;
}
...
function maybeStartLiveSidecar(input: HookInput): string {
  if (process.env.AGENT_AWARENESS_CODEX_LIVE !== '1') return '';
  ...
  if (!appServerUrl || !runId || !runtimeDir) return '';
  ...
  try {
    ...
    if (!threadId) delete sidecarEnv.CODEX_THREAD_ID;
