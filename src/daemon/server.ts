...
type SessionLifecycleStatus = 'online' | 'busy' | 'idle' | 'unknown' | 'offline';
...
function upsertSession(
  sessionId: string,
  updates: Partial<Pick<SessionRecord, 'provider' | 'status' | 'connected' | 'reason'>> = {},
): SessionRecord {
  const now = Date.now();
...
function markHeartbeat(sessionId: string): void {
  const now = Date.now();
  const existing = sessionRecords.get(sessionId);
  if (!existing) {
...
function deriveSessionStatus(record: SessionRecord, now = Date.now()): SessionLifecycleStatus {
  if (record.status === 'offline') return 'offline';
  if (record.connected && now - record.lastHeartbeatAtMs > HEARTBEAT_MISS_GRACE_MS) {
    return 'unknown';
  }
  if (record.status === 'busy' || record.status === 'idle') return record.status;
...
function serializeSession(record: SessionRecord, now = Date.now()): Record<string, unknown> {
  return {
...
function parseStatus(value: unknown): SessionLifecycleStatus | null {
    ...
    try {
      ...
      const alive = await new Promise<boolean>(resolve => {
        const req = httpRequest(
          { host: '127.0.0.1', port: existing.port, path: '/health', method: 'GET', timeout: 2000 },
