...
export function applyInjectionPolicy(inputs, options) {
    ...
    for (const fact of candidates) {
        if (fact.channel !== 'always')
            continue;
        if (previousSeen[fact.fingerprint])
            continue;
...
function normalizeFacts(inputs) {
    const facts = [];
    let order = 0;
...
function collapseLines(lines, severity) {
    if (lines.length <= 1)
        return lines;
    if (severity === 'info') {
        return [lines[0]];
...
function defaultChannelForSeverity(_severity) {
    return 'always';
}
