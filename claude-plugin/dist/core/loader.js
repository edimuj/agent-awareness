...
async function loadFromDirectory(dir, source, errors) {
    ...
    for (const entry of entries) {
        ...
        try {
            ...
            errors.push({ source: label, error: `Failed to load: ${err.message}` });
        }
    }
    return plugins;
}
/** Scan node_modules for agent-awareness-plugin-* packages. */
async function loadNpmPlugins(errors) {
    const plugins = [];
    let entries;
    try {
...
async function loadGlobalNpmPlugins(errors) {
...
async function resolvePackageEntry(pkgDir) {
    try {
...
async function ensureEsmPackageJson(dir) {
...
function validatePlugin(obj) {
    ...
    for (const hook of ['onInstall', 'onUninstall', 'onStart', 'onStop']) {
