...
async function getMcpEntry(key) {
    return runCodex(['mcp', 'get', key]);
}
function isNpxCachePath(path) {
    return path.includes('/.npm/_npx/')
        || path.includes('\\npm\\_npx\\');
}
export async function resolveServerScript() {
    try {
...
export async function codexMcpInstall() {
    let current;
    let legacy;
    ...
    if (legacy.code === 0) {
        ...
        if (removedLegacy.code !== 0) {
            ...
            if (removedLegacy.stderr.trim())
                console.warn(removedLegacy.stderr.trim());
        }
        else {
    ...
    if (added.stdout.trim())
        console.log(added.stdout.trim());
}
export async function codexMcpUninstall() {
