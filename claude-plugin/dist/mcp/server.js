...
import { initStateDir, STATE_DIR, writeTickerPid, clearTickerPid, saveChannelSeen, clearChannelSeen, loadState } from "../core/state.js";
import { setupTicker, tick } from "../daemon/tick-loop.js";
...
async function main() {
    ...
    server.setRequestHandler(CallToolRequestSchema, async (request) => {
        if (request.params.name === 'awareness_doctor') {
            return {
                content: [{ type: 'text', text: await runDoctor(registry, errors) }],
            };
        }
        return {
...
async function startInternalTicker(server) {
    ...
    const onResult = async (pluginName, text) => {
        const fingerprint = hashFingerprint(`${pluginName}:${text}`);
        if (channelSeen.get(fingerprint) === text)
            return;
    ...
    const checkSessionReset = async () => {
        try {
            ...
            const meta = state.__agent_awareness_meta_claude_code;
...
async function runDoctor(registry, loadErrors) {
    const lines = ['agent-awareness doctor', ''];
    let globalRoot = null;
    try {
